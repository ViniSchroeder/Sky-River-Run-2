...
function update(delta) {
  ...
  state.obstacles = state.obstacles.filter(
    (item) => !item.destroyed && item.y < height + 90,
  );
...
function drawPlayer() {
  ...
  context.beginPath();
  context.moveTo(0, -32);
  context.quadraticCurveTo(9, -21, 9, 17);
  context.lineTo(-9, 17);
  context.quadraticCurveTo(-9, -21, 0, -32);
  context.fill();

  context.fillStyle = "#b82222";
  context.beginPath();
  context.moveTo(-35, 2);
  context.lineTo(35, 2);
  ...
  context.fillStyle = "#3b79a8";
  context.beginPath();
  context.ellipse(0, -11, 5, 9, 0, 0, Math.PI * 2);
  context.fill();
...
function draw() {
  ...
  for (const bridge of state.bridges) drawBridge(bridge);
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) stopEngineSound();
